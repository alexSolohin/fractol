...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/03/06 16:31:49 by user              #+#    #+#             */
/*   Updated: 2020/03/09 20:22:52 by user             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		perpendicular(t_fractol *fractol)
{
	fractol->iteration = 0;
...
void			perpendicular_pthread(t_fractol *fractol)
{
	...
	while (i < THREADS)
	{
		...
		if (pthread_create(&threads[i], NULL, (void *(*)(void *))perpendicular_draw, (void *)&tab[i]))
			exit(0);
		i++;
	}
	while (i-- > 0)
